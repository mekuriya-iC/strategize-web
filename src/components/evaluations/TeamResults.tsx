...
export default function TeamResults() {
  ...
  const [liveCompetencyScores, setLiveCompetencyScores] = useState<
    Record<string, { score: number; completed: boolean }>
  >({});
  ...
  const { results: performanceResults, loading: performanceLoading } =
    useAggregatePerformanceResults({
      page: 1,
      limit: 1000,
  ...
  const calculateEmployeeCompetencyScore = async (employeeId: string) => {
    if (!activeCycle?.evaluationCycleId) return { score: 0, completed: false };
    ...
    const assessments =
      assessmentsData?.competencyAssessments?.items?.filter(
        (assessment: any) =>
          assessment.status === "SUBMITTED" ||
          assessment.status === "COMPLETED",
      ) || [];

    if (assessments.length === 0) return { score: 0, completed: false };

    const { data: weightsData } = await client.query({
      query: GET_DEFAULT_WEIGHTS,
      variables: {
        evaluationCycleId: activeCycle.evaluationCycleId,
        evaluateeUserId: employeeId,
      },
    ...
    for (const weight of weightsData?.getDefaultWeights?.weights || []) {
      weights.set(
        String(weight.relationType).toLowerCase(),
        Number(weight.weightPercent),
      );
    }
    ...
    for (const assessment of assessments) {
      ...
      const avg =
        responses.reduce(
          (sum: number, response: any) => sum + Number(response.rating || 0),
          0,
        ) / responses.length;
      const relationWeight =
        weights.get(String(assessment.relationType).toLowerCase()) ?? 0;

      if (relationWeight > 0) {
        weightedSum += avg * relationWeight;
  ...
  useEffect(() => {
    ...
    const calculateLiveScores = async () => {
      if (!activeCycle?.evaluationCycleId || targetEmployees.length === 0) {
        setLiveCompetencyScores({});
        return;
      }
        ...
        const entries = await Promise.all(
          targetEmployees.map(
            async (employee: any) =>
              [
                employee.employeeId,
                await calculateEmployeeCompetencyScore(employee.employeeId),
              ] as const,
          ),
        );

        if (!cancelled) {
          setLiveCompetencyScores(Object.fromEntries(entries));
        }
      } finally {
        if (!cancelled) setLiveScoresLoading(false);
      }
    };
    ...
    targetEmployees.map((e: any) => e.employeeId).join(","),
  ]);

  // Match target employees with aggregate results, falling back to live competency results.
  const teamMembers = targetEmployees.map((employee: any) => {
    ...
    const liveCompetency = liveCompetencyScores[employee.employeeId];
    const competencyScore =
      performance?.competencyScore || liveCompetency?.score || 0;
    const overallScore = performance?.aggregateScore || competencyScore;
    ...
    return {
      ...
      status: performance || liveCompetency?.completed ? "Done" : "Pending",
    };
  });
    ...
    completedMembers.length > 0
      ? (
          completedMembers.reduce(
            (sum: number, m: any) => sum + m.scores.overall,
            0,
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-1">
            {activeCycle?.name || "Current Cycle"} · {filteredMembers.length}{" "}
            members shown
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            className="h-9 rounded-xl border-gray-100 text-gray-600 font-bold text-[10px] uppercase tracking-wider gap-2"
          >
          ...
          <Button
            size="sm"
            className="h-9 rounded-xl bg-indigo-600 hover:bg-indigo-700 font-bold text-[10px] uppercase tracking-wider gap-2 shadow-lg shadow-indigo-100"
          >
                    ...
                    filteredMembers.map((member: any) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-4 text-center">
                          <span
                            className={`text-sm ${getScoreColor(member.scores.competency)}`}
                          >
                        ...
                        <td className="py-4 px-4 text-center">
                          <span
                            className={`text-sm ${getScoreColor(member.scores.individualKpi)}`}
                          >
                        ...
                        <td className="py-4 px-4 text-center">
                          <span
                            className={`text-sm ${getScoreColor(member.scores.sharedKpi)}`}
                          >
                        ...
                        <td className="py-4 px-4 text-center">
                          <div className="inline-flex flex-col items-center">
                            <span
                              className={`text-sm font-bold ${getScoreColor(member.scores.overall)}`}
                            >
                        ...
                        <td className="py-4 px-4 text-center">
                          <Badge
                            className={`text-[10px] font-bold uppercase tracking-widest border-none px-2 py-0.5 h-5 ${
                              member.status === "Done"
