...
export default function CreateKPIForm({
    ...
    const canSubmit = useMemo(() => {
        ...
        return true;
    }, [existingWeight, formData.weight, isSubmitting, hasValidBasicFields, strategicYear, hasAnnualTarget, isCorporate, hasQuarterlyData]);
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={handleFormSubmit}>
                <div className="space-y-6">
                    ...
                    <div className={`p-4 rounded-lg mb-6 border ${existingWeight + (parseFloat(formData.weight) || 0) > 100 ? 'bg-red-50 border-red-200' : 'bg-zinc-50 border-zinc-200'}`}>
                        <div className="flex justify-between items-center mb-2">
                            <span className="text-sm font-medium text-zinc-700">Objective Weight Allocation</span>
                            <span className={`text-sm font-bold ${existingWeight + (parseFloat(formData.weight) || 0) > 100 ? 'text-red-600' : 'text-zinc-900'}`}>
                                {(existingWeight + (parseFloat(formData.weight) || 0)).toFixed(1)}% / 100%
                            </span>
                        </div>
                        <div className="w-full bg-zinc-200 rounded-full h-2 overflow-hidden">
                            <div
                                ...
                                style={{ width: `${Math.min(100, existingWeight + (parseFloat(formData.weight) || 0))}%` }}
                            />
                        </div>
                    ...
                    {strategicYear && (
                        <div className="border p-4 rounded-md bg-gray-50">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="font-medium text-gray-900">Strategic Period Target</h3>
                            ...
                            <div className="bg-white p-3 rounded border shadow-sm">
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        onChange={(e) => setAnnualTargets(prev => ({ ...prev, [strategicYear]: e.target.value }))}
                                    ...
                                    <span className="absolute right-3 top-2 text-gray-400 text-xs font-medium pointer-events-none">
                                        {formData.unitType === "PERCENT" ? "%" : "million ETB"}
                                    </span>
                                </div>
                        ...
                        <div className="border p-4 rounded-md shadow-sm">
                            ...
                            <QuarterlyBreakdown
                                ...
                                strategicTargetsById={{
                                    ...
                                    "new": strategicYear ? {
                                        [strategicYear]: parseFloat(annualTargets[strategicYear] || "0")
                                    } : {}
                                }}
                            />
                        </div>
