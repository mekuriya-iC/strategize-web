...
export function GenerateScheduleDialog({ open, onOpenChange, onGenerated }: GenerateScheduleDialogProps) {
  ...
  const { activeStrategicPlan, strategicPeriods, loading: periodsLoading, refetch: refetchPlanContext } = useActiveStrategicPlanPeriods();

  useEffect(() => {
    if (!open) return;
    void refetchPlanContext();
  }, [open, refetchPlanContext]);
  ...
  const previewSchedule = async () => {
    if (!activeStrategicPlan?.strategicPlanId) {
      toast.error("No active strategic plan is available. Activate a plan, then try again.");
      return;
    }
    try {
      const result = await loadPreview({ variables: { input } });
      if (result.error) {
        toast.error(result.error.message || "Could not preview this schedule.");
        return;
      }
      setStep("preview");
    } catch (error: unknown) {
      toast.error(error instanceof Error ? error.message : "Could not preview this schedule.");
    }
  };
