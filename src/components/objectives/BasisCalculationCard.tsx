  ...
  return (
    <section className="space-y-4 rounded-lg border border-blue-200 bg-blue-50/40 p-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h3 className="font-semibold text-slate-900">Basis calculation</h3>
          <p className="text-sm text-slate-600">
            {isPercent
              ? "Choose whether users enter the percentage result manually or calculate it from a numerator and denominator."
              : "Choose whether users enter the ratio result manually or calculate it from numerator and denominator components."}
          </p>
        </div>
      </div>
      ...
      >
        <label className="flex cursor-pointer gap-2 rounded-md border bg-white p-3">
          ...
          <span>
            <span className="flex items-center gap-1.5 text-sm font-medium">
              <PenLine className="h-4 w-4" /> Manual result
            </span>
        ...
        <label className="flex cursor-pointer gap-2 rounded-md border bg-white p-3">
          ...
          <span>
            <span className="flex items-center gap-1.5 text-sm font-medium">
              <Calculator className="h-4 w-4" /> Enter basis directly
            </span>
            <span className="mt-1 block text-xs text-slate-500">
        ...
        <label className="flex cursor-pointer gap-2 rounded-md border bg-white p-3">
          ...
          <span>
            <span className="flex items-center gap-1.5 text-sm font-medium">
              <Link2 className="h-4 w-4" /> Link KPI
            </span>
            <span className="mt-1 block text-xs text-slate-500">
          ...
          <div className="rounded-md border border-blue-200 bg-white p-3 text-sm">
            ...
            {!isPercent && targetNumber >= 1 && (
              <p className="mt-1 text-xs text-amber-700 bg-amber-50 p-2 rounded">
                💡 Tip: For "1 out of {Math.round(1/targetNumber)}" ratio, enter <strong>{(1/Math.round(1/targetNumber)).toFixed(4)}</strong> as the target instead of {targetNumber}.
              </p>
            )}
      ...
      {source === "LINKED_KPI" && (
        <div className="space-y-2 border-t border-blue-100 pt-4">
          <Label>Additive basis KPI</Label>
          <Select
          ...
          <p className="text-xs text-slate-500">
            The linked KPI supplies the additive denominator. {targetDisplay} is
            retained for every child while results roll up by denominator weight.
          </p>
