...
export default function CreateSessionDialog({
  ...
  const { data: existingSessionsData, loading: sessionsLoading } = useQuery(GET_CHECKINOUT_SESSIONS, {
    variables: { 
  ...
  const { data: periodsData, loading: periodsLoading } = useQuery(GET_STRATEGIC_PERIODS, {
    variables: { page: 1, limit: 100 },
    fetchPolicy: 'cache-and-network',
  });

  const [createSession] = useMutation(CREATE_CHECKINOUT_SESSION, {
    refetchQueries: [GET_CHECKINOUT_SESSIONS],
  });
  ...
  const employeesFromDepartments = useMemo(() => {
    ...
    departments.forEach((dept: any) => {
      ...
      if (dept.head?.employeeId === currentUserId) {
        console.log(`  ✅ Current user IS head of ${dept.name}`);
        console.log(`  👥 Employees in department:`, dept.employees?.length || 0);
        
        // Add all employees from this department
        dept.employees?.forEach((emp: any) => {
          if (emp.employeeId && emp.employeeId !== currentUserId) {
            // Don't include the manager themselves
            console.log(`    Adding employee: ${emp.fullName} (${emp.employeeId})`);
  ...
  const handleSelectAll = () => {
    if (selectedEmployees.length === availableEmployees.length) {
      ...
      setSelectedEmployees(availableEmployees.map((emp: any) => emp.employeeId));
    }
  };
    ...
    try {
      for (const employeeId of selectedEmployees) {
        try {
          ...
          const employee = directReports.find((emp: any) => emp.employeeId === employeeId);
          if (employee) {
            successfulEmployees.push(employee.fullName);
          }
        } catch (error: any) {
          console.error(`Failed to create session for employee ${employeeId}:`, error);
          errorCount++;
      ...
      if (errorCount === 0) {
        const durationText = durationDays === 7 ? '1 week' : durationDays === 14 ? '2 weeks' : `${durationDays} days`;
        ...
        if (successfulEmployees.length <= 3) {
          toast.success(`Check-in period created for ${successfulEmployees.join(', ')} (${dateRange})`);
        } else {
          toast.success(`Check-in period created for ${successCount} employees (${dateRange}, ${durationText})`);
        }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh]">
        <DialogHeader>
          ...
          <p className="text-sm text-gray-600 mt-1">
            Set up a check-in period for your team members. Each employee will get their own session to track their tasks.
          </p>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="sprintTitle">
              Sprint Title <span className="text-gray-500 text-xs">(Optional)</span>
            </Label>
            <Input
              id="sprintTitle"
            ...
            <p className="text-xs text-gray-500">
              Give this sprint a memorable name. If left empty, it will be auto-generated.
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={strategicPeriodId} onValueChange={setStrategicPeriodId}>
              <SelectTrigger>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">Must be today or future date</p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">Flexible duration (1 week, 2 weeks, etc.)</p>
              </div>
            </div>
            ...
            {sessionDuration && (
              <div className="flex items-center gap-2 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-semibold text-indigo-600">{sessionDuration}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {(employeesLoading || departmentsLoading || divisionsLoading || sessionsLoading) ? (
              ...
              <div className="flex flex-col items-center justify-center h-32 border rounded-lg bg-amber-50">
                ...
                <p className="text-sm text-amber-700 font-medium">All team members have active sessions</p>
                <p className="text-xs text-amber-600 mt-2 text-center px-4 max-w-sm">
                  Close existing sessions before creating new ones. Only one active session per employee is allowed.
                </p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center h-32 border rounded-lg bg-gray-50">
                <Users className="h-8 w-8 text-gray-400 mb-2" />
                <p className="text-sm text-gray-600 font-medium">No team members found</p>
            ...
            {selectedEmployees.length > 0 && (
              <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                ...
                <span className="text-sm text-gray-700">
                  <span className="font-semibold text-green-600">{selectedEmployees.length}</span> employee{selectedEmployees.length !== 1 ? 's' : ''} selected
                  {sessionDuration && <span className="text-gray-500"> • {sessionDuration} period</span>}
                </span>
              </div>
            )}
          </div>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={creating || selectedEmployees.length === 0 || !strategicPeriodId || !weekStartDate || !weekEndDate}
          ...
          >
            {creating ? (
              ...
              <>
                Create Period for {selectedEmployees.length} Employee{selectedEmployees.length !== 1 ? 's' : ''}
              </>
            )}
          </Button>
        </DialogFooter>
