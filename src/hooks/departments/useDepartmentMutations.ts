...
export const useDepartmentMutations = () => {
  ...
  const [addEmployeeToDepartmentMutation, { loading: addEmployeeLoading }] = useMutation(ADD_EMPLOYEE_TO_DEPARTMENT, {
      onError: (error) => {
        deptLogger.error("Failed to add employee to department:", error.message);
      },
    });

  const [removeEmployeeFromDepartmentMutation, { loading: removeEmployeeLoading }] = useMutation(REMOVE_EMPLOYEE_FROM_DEPARTMENT, {
      onCompleted: (data) => {
