...
export default function MyEvaluations() {
  ...
  const filteredAssessments = assessments?.filter((assessment: any) => {
    ...
    if (filterStatus === "pending")
      return assessment.status === EvaluationStatus.NOT_STARTED;
    if (filterStatus === "in-progress")
      return assessment.status === EvaluationStatus.IN_PROGRESS;
  ...
  const counts = {
    ...
    pending:
      assessments?.filter((a: any) => a.status === EvaluationStatus.NOT_STARTED)
        .length || 0,
    inProgress:
      assessments?.filter((a: any) => a.status === EvaluationStatus.IN_PROGRESS)
        .length || 0,
    completed:
      assessments?.filter(
        (a: any) =>
      ...
      <div className="space-y-4">
        {!filteredAssessments || filteredAssessments.length === 0 ? (
          ...
          filteredAssessments.map((assessment: any) => {
            ...
            const isSelf =
              assessment.relationType === EvaluationRelationType.SELF;
            const isCompleted =
              assessment.status === EvaluationStatus.COMPLETED ||
              assessment.status === EvaluationStatus.SUBMITTED;
            ...
            return (
              <Card
                ...
                className="border-none shadow-sm bg-white rounded-2xl overflow-hidden hover:shadow-md transition-shadow"
              >
                    ...
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-center gap-3">
                          ...
                          <Badge
                            className={`text-[10px] font-bold uppercase tracking-tighter px-1.5 py-0 h-4 border-none ${
                              assessment.relationType.toLowerCase() === "self"
                                ? "bg-indigo-50 text-indigo-600"
                                : assessment.relationType.toLowerCase() ===
                                    "peer"
                      ...
                      <Button
                        onClick={() =>
                          handleStartEvaluation(
                            assessment.competencyAssessmentId,
                          )
                        }
