...
import { ChevronDown, ChevronUp, Users, PlusIcon, SearchIcon, FilterIcon, ArrowLeft, Target, CheckCircle2, AlertCircle, TrendingUp, Calendar } from "lucide-react";
...
function EmployeeTaskCard({ 
  ...
  const canEdit = isCurrentUser || isManagerOfThisEmployee;

  return (
    <div className={cn(
      "bg-white dark:bg-gray-800 rounded-xl border transition-all duration-300 overflow-hidden mb-6 shadow-sm",
      isExpanded ? "ring-1 ring-[#3838EC]/30 border-[#3838EC]/20 shadow-md" : "border-gray-200 dark:border-gray-700 hover:border-[#3838EC]/30 hover:shadow-md"
    )}>
      <div 
        className={cn(
          "p-5 flex items-center justify-between cursor-pointer group",
          isExpanded ? "bg-[#3838EC]/5 dark:bg-[#3838EC]/10" : "hover:bg-gray-50/50 dark:hover:bg-gray-700/30"
        )}
        onClick={onToggle}
      >
          ...
          <div className="relative">
            <Avatar className="h-12 w-12 border-2 border-white dark:border-gray-700 shadow-sm">
              ...
              <AvatarFallback className="bg-gradient-to-br from-[#3838EC] to-[#5B5BF7] text-white font-bold">
                {session.employee?.fullName?.split(' ').map((n: string) => n[0]).join('')}
              </AvatarFallback>
            </Avatar>
            <div className={cn(
              "absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white dark:border-gray-800 shadow-sm",
              session.overallStatus === 'ACTIVE' ? "bg-green-500" : "bg-gray-400"
            )} />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-0.5">
              ...
              {isManagerOfThisEmployee && !isCurrentUser && (
                <Badge variant="outline" className="text-[10px] h-4 border-[#3838EC] text-[#3838EC] bg-[#3838EC]/5 font-bold uppercase tracking-wider">
                  My Team
                </Badge>
              )}
            </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge className={cn(
              ...
              session.overallStatus === 'COMPLETED' ? 'bg-green-100 text-green-700 border border-green-200' :
              session.overallStatus === 'ACTIVE' ? 'bg-blue-100 text-blue-700 border border-blue-200' :
          ...
          <div className={cn(
            ...
            isExpanded ? "bg-[#3838EC]/10 text-[#3838EC]" : "text-gray-400 group-hover:bg-gray-100 dark:group-hover:bg-gray-700"
          )}>
      ...
      {isExpanded && (
        <div className="bg-white dark:bg-gray-800">
          <div className="px-5 py-4 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between bg-gray-50/50 dark:bg-gray-900/10">
            ...
            {isManagerOfThisEmployee && !isCurrentUser && (
              <span className="text-[11px] text-[#3838EC] font-bold italic">
                Manager Control: You can edit or delete team tasks
              </span>
            )}
          </div>
          ...
          <div className="p-0 overflow-x-auto">
            {tasksLoading ? (
              <div className="p-12 text-center">
                <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-[#3838EC] border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]" role="status">
                  <span className="!absolute !-m-px !h-px !w-px !overflow-hidden !whitespace-nowrap !border-0 !p-0 ![clip:rect(0,0,0,0)]">Loading...</span>
                </div>
                <p className="mt-4 text-sm text-gray-500 font-medium">Fetching tasks...</p>
              </div>
            ) : tasks.length === 0 ? (
              <div className="p-16 text-center">
                ...
                <h5 className="text-gray-900 dark:text-white font-semibold mb-1">No tasks found</h5>
                <p className="text-sm text-gray-500">This employee hasn't added any tasks to this session yet.</p>
              </div>
            ) : (
              <CheckInTable
                tasks={tasks}
...
export default function CheckInPage() {
  ...
  const [createMySession, { loading: creatingMySession }] = useMutation(CREATE_CHECKINOUT_SESSION);

  // Get check-in sessions for current user (as employee)
  const { data: employeeData, loading: employeeLoading, refetch: refetchEmployeeSessions } = useQuery(GET_CHECKINOUT_SESSIONS, {
    variables: {
  ...
  const { data: supervisorData, loading: supervisorLoading, refetch: refetchSupervisorSessions } = useQuery(GET_CHECKINOUT_SESSIONS, {
    variables: {
  ...
  const currentSession = useMemo(() => {
    if (selectedSessionId) {
      return allSessions.find((s: any) => s.checkinoutSessionId === selectedSessionId);
    }
  ...
  const isManagerMode = useMemo(() => {
    ...
    return currentSession.supervisor?.employeeId === currentUser.employeeId || supervisedSessions.length > 0;
  ...
  const teamSessions = useMemo(() => {
    ...
    const supervisedThisWeek = supervisedSessions.filter((s: any) => 
      normalizeDate(s.weekStartDate) === weekStart
    );
    
    // 2. Get the current user's own session for this week (even if their supervisor is different)
    const myOwnSession = employeeSessions.find((s: any) => 
      normalizeDate(s.weekStartDate) === weekStart
    );

    // Combine them, ensuring the current user's session is at the top
    const combined = [...supervisedThisWeek];
    if (myOwnSession && !combined.find(s => s.checkinoutSessionId === myOwnSession.checkinoutSessionId)) {
  ...
  const mySessionId = useMemo(() => {
    if (!currentSession || !currentUser || !employeeSessions.length) return null;
    
    const weekStart = normalizeDate(currentSession.weekStartDate);
    
    const mySession = employeeSessions.find((s: any) => 
      normalizeDate(s.weekStartDate) === weekStart
    );
  ...
  const teamStatistics = useMemo(() => {
    ...
    return {
      ...
      completedSessions,
      completionRate: Math.round((completedSessions / teamSessions.length) * 100),
    };
  ...
  const handleAddMyTaskClick = async () => {
    ...
    const strategicPeriodId = currentSession?.strategicPeriod?.strategicPeriodId;
              ...
              <SelectContent>
                {allSessions.map((session: any) => (
                  <SelectItem key={session.checkinoutSessionId} value={session.checkinoutSessionId}>
                    {session.employee?.fullName} - Week of {new Date(session.weekStartDate).toLocaleDateString()}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
            ...
            {isManagerMode && <Badge className="bg-[#3838EC]/10 text-[#3838EC] border-[#3838EC]/20 gap-1"><Users className="w-3 h-3"/> Team View</Badge>}
          </h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {currentSession && (
              <>
                Week: {new Date(currentSession.weekStartDate).toLocaleDateString()} - {new Date(currentSession.weekEndDate).toLocaleDateString()}
              </>
            )}
          </p>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-all duration-300 group">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge variant="outline" className="bg-blue-50/50 text-[#3838EC] border-blue-100">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-all duration-300 group">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex flex-col items-end">
                  <span className="text-2xl font-bold text-green-600">{statistics.kpiMetPercentage}%</span>
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
              ...
              <div className="mt-4 pt-4 border-t border-gray-50 dark:border-gray-700/50 flex items-center justify-between text-xs text-gray-400 font-medium">
                {isManagerMode && teamStatistics ? (
                  <>
                    ...
                    <span className="text-green-600 font-bold">{teamStatistics.completedSessions}/{teamStatistics.teamSize}</span>
                  </>
                ) : (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-6 shadow-sm hover:shadow-md transition-all duration-300 group">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex flex-col items-end">
                  <span className="text-2xl font-bold text-amber-500">{statistics.kpiUnmetPercentage}%</span>
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">
          ...
          {isManagerMode ? (
              ...
              <div className="space-y-4">
                {teamSessions.map((session: any) => (
                  <EmployeeTaskCard
                    ...
                    session={session}
                    isExpanded={expandedSessions[session.checkinoutSessionId] ?? (session.employee?.employeeId === currentUser?.employeeId)}
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
              ...
              <Button
            onClick={() => {
              setTargetSessionId(currentSession?.checkinoutSessionId || null);
              setIsAddTaskOpen(true);
            }}
          ...
          >
            ...
            <div className="flex-1 flex flex-col">
              ...
              <CheckInTable
                ...
                createdDate={new Date(currentWeekDataWithTasks?.createdAt || currentSession.weekStartDate)}
