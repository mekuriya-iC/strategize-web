      ...
      for (const assignment of assignments) {
        ...
        if (!existingObjective) {
          ...
          if (created?.objectiveId) {
            ...
            const typeLabel =
              assignment.assigneeType === "DIVISION"
                ? "Division"
        ...
        if (existingObjective && targetObjectiveId) {
          const existingKpiNames = new Set(
            existingObjective.kpis?.map((k: any) => k.name),
          );
        ...
        if (freshObjective) {
          for (const sourceKpiId of assignment.kpis) {
            ...
            if (childKpi) {
              ...
              console.warn(
                `[useAssignmentActions] Could not find child KPI for source KPI ${sourceKpiId}`,
              );
            }
          }
        }
