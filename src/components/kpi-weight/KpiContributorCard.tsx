...
import type { KpiWithContributors, KpiContributor } from '@/hooks/kpi-weight/useHierarchicalKpiContributions';
...
export function KpiContributorCard({ kpi, onContributorClick, showDrillDown = true }: KpiContributorCardProps) {
  ...
  const achievementPercentage = parseFloat(String(kpi.achievementPercentage)) || 0;
    ...
    <Card className="overflow-hidden">
      <CardHeader className="bg-gradient-to-r from-indigo-50 to-blue-50 dark:from-indigo-950 dark:to-blue-950">
        <div className="flex items-start justify-between">
          ...
          <Badge
            variant={achievementPercentage >= 100 ? 'default' : achievementPercentage >= 80 ? 'secondary' : 'destructive'}
            className="text-base px-3 py-1"
          >
            {achievementPercentage.toFixed(1)}%
          </Badge>
        </div>
        <Progress value={Math.min(achievementPercentage, 100)} className="h-2 mt-3" />
      </CardHeader>
      ...
      <CardContent className="pt-6">
        {!kpi.hasContributors ? (
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-xs mt-1">This KPI is directly assigned without breakdown</p>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            {kpi.contributors.map((contributor) => {
              const contribAchievementPercentage = parseFloat(String(contributor.achievementPercentage)) || 0;
              const contribParentWeight = parseFloat(String(contributor.parentWeightAllocation)) || 0;
              const contribTargetValue = parseFloat(String(contributor.targetValue)) || 0;
              const contribAchievedValue = parseFloat(String(contributor.achievedValue)) || 0;
              const contribWeightContribution = parseFloat(String(contributor.weightContribution)) || 0;
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h5 className="font-medium truncate">{contributor.contributorName}</h5>
                          <Badge variant="outline" className="text-xs">
                            {contributor.contributorType}
                          </Badge>
                        </div>
                        <div className="flex items-center gap-3 text-xs text-muted-foreground mt-1">
                          <span>{contribAchievedValue.toLocaleString()} / {contribTargetValue.toLocaleString()}</span>
                          <span className={getContributorColor(contribAchievementPercentage)}>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {showDrillDown && onContributorClick && contributor.contributorType !== 'EMPLOYEE' && (
                        <Button
                          variant="ghost"
                          size="sm"
